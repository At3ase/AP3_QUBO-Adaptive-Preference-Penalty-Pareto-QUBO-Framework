...
```
H_penalty = λ_sum × (S_const + S_var − 100)²
          = λ_sum × (S_var − 75)²
          = λ_sum × [S_var² − 150×S_var + 5625]
```

其中 `S_var²` 展开后：
- 自平方项：`x_i² = x_i`（二元变量）→ 线性项
- 交叉项：`2×coef_i×coef_j×x_i×x_j` → QUBO二次项
...
| **P1（强偏好）** | 相结构（VEC） | ω_phase = 0.1~0.5 | 希望成形为BCC+FCC双相固溶体 |
| **P2（弱偏好）** | 成本/稀缺性约束 | ω_cost = 0.01~0.1 | 倾向低成本元素组合 |

总惩罚项：
```
H_penalty = ω_sum·λ_sum·H_sum + ω_phase·λ_phase·H_phase + ω_cost·λ_cost·H_cost
```
...
```
VEC = Σ c_i · VEC_i
VEC_i: Al=3, Co=9, Cr=6, Fe=8, Ni=10, C=4（间隙近似）
```
VEC为经验判据，不精确但方向性有效。设为**P1软约束而非目标函数**：
```
H_VEC = (VEC − 7.3)²  // 目标区间7.0~7.6（双相区），中心点7.3
```
权重：`ω_VEC ≈ 0.1 × ω_ΔH`，避免与f₁的ΔH_mix目标重复。

> **VEC风险提示**：C的VEC=4为间隙原子经验近似，C-HEA体系实验验证有限，仅作为方向性引导。

> **关于ΔH_mix的重叠处理**：f₁目标函数已直接负责热力学稳定性（最小化ΔH_mix），因此P1约束中**不再重复包含ΔH_mix软引导**。避免同一物理量既被目标拉动又被约束回拉，造成QUBO内部对抗。VEC作为独立的相结构判据（FCC/BCC选择）保留。
...
────────────────────────────────────
输入：初始λ⁽⁰⁾ = (0.1, 0.0, 0.0)  // (sum, phase, cost)
      α=0.8, γ=0.5, T_max=15
      偏好权重 ω = (1.0, ω_phase, ω_cost)
输出：优化解 x*，自适应λ*，收敛历史
...
| **λ warm-start** | 复用最近邻PenaltyFlex终值 | 相邻权重共享相似约束难度 |
...
| **内层PenaltyFlex** | 3.2节 | 惩罚系数 (λ_sum,λ_phase,λ_cost) | **约束满足率**（可行性、精确度） |
