...
| **P2（弱偏好）** | 成本/稀缺性约束 | λ_cost：PenaltyFlex自适应 | 倾向低成本元素组合 |
...
总惩罚项（P0硬约束 + P1/P2软约束）：
```
H_total = H_obj + λ_sum_fixed·H_sum + ω_carbide·λ_carbide·H_carbide + ω_cost·λ_cost·H_cost

// λ_sum_fixed = 10~20（固定常数，不参与自适应）
// λ_carbide, λ_cost 由 PenaltyFlex 自适应控制
```
...
#### 3.2.4 自适应更新规则（PenaltyFlex）

PenaltyFlex仅控制P1/P2的λ（carbide, cost），P0的λ_sum为固定常数不参与自适应。
...
**更新公式**（仅适用于carbide, cost）：
```
λ_j^(t+1) = λ_j^(t) × exp( α · tanh(v̄_j − ε_j) )    j ∈ {carbide, cost}
```
...
输入：λ_sum_fixed = 10~20  // P0硬约束固定常数，不参与自适应
      初始λ_soft⁽⁰⁾ = (0.0, 0.0)  // (carbide, cost)
      α=0.8, γ=0.1, T_max=15
      偏好权重 ω = (ω_carbide, ω_cost)
输出：优化解 x*，自适应λ_soft*，收敛历史

1. FOR t = 1, ..., T_max:
2.     组装QUBO：
        H = H_obj + λ_sum_fixed·H_sum + ω_carbide·λ_carbide⁽ᵗ⁾·H_carbide + ω_cost·λ_cost⁽ᵗ⁾·H_cost
3.     量子求解：solutions = kaiwu.solve(H)
4.     TOP-10解解码：{(c_k, v_k)}，v_k = 约束违反度
5.     目标值评估：f_k = (f₁(c_k), f₂(c_k), f₃(c_k))
6.     
7.     FOR 每个约束j ∈ {carbide, cost}:  // P0已固定，不参与自适应
...
| **内层PenaltyFlex** | 3.2节 | 惩罚系数 λ_carbide, λ_cost（λ_sum=10~20固定） | **约束满足率**（可行性、精确度） |
