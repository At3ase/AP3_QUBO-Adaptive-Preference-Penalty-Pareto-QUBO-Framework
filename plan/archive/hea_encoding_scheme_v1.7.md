...
> **QUBO可实现性说明**：原方案使用 `max(0, c_C − 0.8%)²`（分段函数），但数学验证表明3比特QUBO无法精确表示8点分段函数（7自由度 < 8约束条件），在c_C=1.75%时误差达27.7%。改用纯二次型 `(c_C − 0.8%)²` 可**精确嵌入QUBO**，且对 c_C < 0.8% 产生负贡献（即奖励低C成分），这在物理上是**有利的**——低C固溶区本身即为期望方向。
...
输入：初始λ⁽⁰⁾ = (0.1, 0.0, 0.0)  // (sum, carbide, cost)
      α=0.8, γ=0.1, T_max=15  // γ收紧：收敛需λ变化<10%（原50%过松）
...
11.        ELSE:
12.            IF j == "sum":  // P0对称自适应（v1.7修正：允许λ减小释放探索空间）
...
|------|------|---------|---------|----------|-------------|
| **f₁** | 形成能 | 最小化 | 合金热力学稳定性 | −20 ~ +5 kJ/mol | **✅ 天然二次型**（ΔH_mix） |
...
**物理先验归一化**（粗网格前可用）：
```
f₁^norm = f₁ / 30    // ΔH_mix范围 ~30 kJ/mol
f₂^norm = f₂ / 10    // 密度范围 ~4-10 g/cm³
f₃^norm = f₃ / c_max // c_max = max(f₃) 预计算
```

**动态归一化**（粗网格完成后切换，更精确）：
```
f_i^norm = (f_i − f_i^min) / (f_i^max − f_i^min + ε)    ε = 1e-6
```
其中 `f_i^min`、`f_i^max` 从粗网格6组求解的可行解中提取。
...
归一化后各目标量纲统一为[0, 1]区间，权重 `w₁, w₂, w₃` 反映用户真实偏好比例。
...
ΔS_mix = −R · Σ c_i · ln(c_i)  // 理想混合熵 R=8.314 J/(mol·K)
Ω = T_m · ΔS_mix / |ΔH_mix|
IF Ω < 1.1:
