...
**展开验证**：
- c_C = 0.25% × Σ_{k=0}^{2} 2^k · x_{35+k}    （3比特：x₃₅~x₃₇）
- c_Cr = 5.0% + 0.25% × Σ_{j=0}^{6} 2^j · x_{14+j}  （7比特：x₁₄~x₂₀）

展开后：
```
H_CCr = 0.194 × Σ_{k=0}^{2} 2^k x_{35+k}                     // 线性项(h_i)
      + 0.00972 × Σ_{k=0}^{2} Σ_{j=0}^{6} 2^{k+j} x_{35+k} x_{14+j}   // 二次交叉项(Q_ij)
```
...
**交叉项系数基数** = 0.00972，Q_{35+k, 14+j} = 基数 × 2^{k+j}

|  | x₁₄(Cr,b₀) | x₁₅(Cr,b₁) | x₁₆(Cr,b₂) | x₁₇(Cr,b₃) | x₁₈(Cr,b₄) | x₁₉(Cr,b₅) | x₂₀(Cr,b₆) |
|--|-----------|-----------|-----------|-----------|-----------|-----------|-----------|
| **x₃₅(C,b₀)** | 0.00972 | 0.0194 | 0.0389 | 0.0778 | 0.156 | 0.311 | 0.622 |
| **x₃₆(C,b₁)** | 0.0194 | 0.0389 | 0.0778 | 0.156 | 0.311 | 0.622 | 1.244 |
| **x₃₇(C,b₂)** | 0.0389 | 0.0778 | 0.156 | 0.311 | 0.622 | 1.244 | 2.488 |

**量级验证**：最大交叉项2.488，在ω_CCr=0.1、λ_CCr=1.0时贡献≈0.25，与f₁^norm ≈ O(0.1~1) **同量级**。

**注入位置**（Python）：
```python
# 在H_CCr前乘以外部权重
omega_CCr = 0.1   # 用户可调（0.1~0.3）
lambda_CCr = PenaltyFlex输出  # 自适应

# 线性项（加到h向量）
h[35] += omega_CCr * lambda_CCr * 0.194
h[36] += omega_CCr * lambda_CCr * 0.388
h[37] += omega_CCr * lambda_CCr * 0.776

# 交叉项（加到Q矩阵，仅填充C-bit × Cr-bit区域，21个非零项）
base = 0.00972 * omega_CCr * lambda_CCr
...
> **与P1 H_carbide的分工**：
> - **P1 H_carbide = (c_C − 0.8%)²**：抑制C的**绝对含量**（单一元素上限）
> - **P2 H_CCr = c_C × c_Cr**：抑制C与Cr的**协同效应**（二元耦合）
> - 两者正交互补：P1对"高C低Cr"场景敏感，P2对"中C高Cr"场景敏感
...
11.        ELSE:
12.            λ_j^(t+1) = λ_j^(t) × exp( α · tanh(v̄_j − ε_j) )
13.   
14.    IF MAX_j |λ_j⁽ᵗ⁺¹⁾ − λ_j⁽ᵗ⁾| / λ_j⁽ᵗ⁾ < γ:
15.        BREAK  // 收敛
16. RETURN best_solution, λ_soft_final
```
...
> 但需重新初始化PenaltyFlex λ为(0.1, 0, 0)。对当前38变量问题增益有限。
...
> 
> **简化说明**：本检验仅检查f₁-f₂平面投影（最强trade-off面）。完整3D凸包检验推荐用于生产环境。f₂-f₃ trade-off较弱，非凸可能性低。
...
| P2约束 | C-Cr耦合最小化 c_C × c_Cr | **✅ 确认 (v1.11新增)** |
| P2权重 | ω_CCr = 0.1~0.3 | **✅ 确认** |
